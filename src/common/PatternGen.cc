...
int PatternGen::layerorder_all[6][6] = {{3,1,5,6,4,2},//layerorder1
        {4,2,5,6,3,1},
        {1,3,6,5,2,4},
        {2,4,6,5,1,3},
        {4,6,2,1,3,5},
...
template <size_t XS_per_CFEB> int PatternGen::GetOutputXStrip(int cfeb, int input_xstrip, int tmb_compile_type) {
  const int Bad_HS = -1;
...
template <size_t XS_per_CFEB> int PatternGen::GetInputXStrip(int output_xstrip, int tmb_compile_type) {
  const int Min_CFEB_in_non_me11 = 0;
  const int Max_CFEB_in_non_me11 = 4;
  const int Min_CFEB_in_region_a = 0;
  const int Max_CFEB_in_region_a = 3;
  ...
  return input_xs;
    ...
    //
template <size_t XS_per_CFEB>  int PatternGen::GetInputCFEBByX(int output_xstrip, int tmb_compile_type) {
  const int Min_CFEB_in_non_me11 = 0;
  const int Max_CFEB_in_non_me11 = 4;
  const int Min_CFEB_in_region_a = 0;
  const int Max_CFEB_in_region_a = 3;
...
bool PatternGen::hits_Generation(int strip, int pattern, int Nhits, unsigned int hits[CSCConstants::NUM_DCFEBS]){
...
void PatternGen::dumptriads(unsigned int triads[CSCConstants::NUM_DCFEBS][3][CSCConstants::NUM_LAYERS]){
...
void PatternGen::finalizetriads(unsigned int (&triads)[CSCConstants::NUM_DCFEBS][3][CSCConstants::NUM_LAYERS], unsigned int (&triads_final)[CSCConstants::NUM_DCFEBS][3][CSCConstants::NUM_LAYERS]){
...
void PatternGen::writetriads(unsigned int (&triads)[CSCConstants::NUM_DCFEBS][3][CSCConstants::NUM_LAYERS],std::vector<std::fstream*> oss){
...
void PatternGen::fillnbytes(std::vector<std::fstream*> oss,unsigned int n){
...
void PatternGen::shufflelayers(unsigned int (&triads)[CSCConstants::NUM_DCFEBS][3][CSCConstants::NUM_LAYERS], int layerset = 1){
...
void PatternGen::PatternGenerator(char * textfile, char * patfiledir) {
   ...
   // text_file.ignore ( std::numeric_limits<std::streamsize>::max(), '\n' );
    ...
    std::cout << "this is the prefix of the file " << prefix << std::endl;
    ...
    for (int i = 0; i < CSCConstants::NUM_DCFEBS; i++){
        std::stringstream ss;
        ss << patfiledir << "/" << prefix <<"_ClctPattern_CFEB" << i << "_tmb"<< tmbtype << ".pat";
